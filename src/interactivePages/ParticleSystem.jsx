...
const ParticleSystem = forwardRef(({ origin = [0, 1, 0] }, ref) => {
    const pointsRef = useRef();
    const texture = useLoader(THREE.TextureLoader, textureURL);

    // === 🎛️ APPEARANCE & BEHAVIOR CONTROLS ===
    const count = 40;
    const spread = 1.0;               // 💨 Horizontal area where particles spawn
    const rise = 6;                   // 🧼 Vertical lift speed
    const drift = 0.3;                // 🌬️ Horizontal drift amount
    const maxOpacity = 0.15;          // 🌫️ Max steam opacity
    const minSize = 2000;             // 🔽 Smallest particle size (in pixels)
    ...
    const { particles, positions, alphas, sizes } = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            const pos = new THREE.Vector3(
                ...
                origin[1] + Math.random() * 0.3 + 0.2, // ☁️ Slight random vertical offset
                origin[2] + (Math.random() - 0.5) * spread
            );

            const velocity = new THREE.Vector3(
                (Math.random() - 0.5) * drift,           // ⬅️➡️ Horizontal drift
                0.015 + Math.random() * rise * 0.4,      // ⬆️ Vertical speed
    ...
    useFrame((_, delta) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // 🌫️ Smooth fade in/out
            const fadeIn = Math.min(1, t * t);               // Smooth, eased fade-in (quadratic)
            const fadeOut = Math.max(0, 1 - (t - 0.5) * 2);  // Second half
            alphaArray[i] = maxOpacity * Math.min(fadeIn, fadeOut);

            // 🔽 Shrink over time (optional)
            sizeArray[i] *= 0.995;
