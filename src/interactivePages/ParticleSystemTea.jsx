...
const ParticleSystemTea = forwardRef(({ origin = [0, 1, 0] }) => {
  ...
  const rise = 32;                  // Vertical lift speed
  ...
  // === Particle Data Initialization (locked with useRef to persist across renders) ===
  ...
  if (!particlesData.current) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const initialLife = 5 + Math.random() * 10;  // Lifetime in seconds (5 - 10)
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Smooth fade in/out
      const fadeIn = Math.min(1, Math.pow(t, 5));
